...
public:
	...
	FSimplePixelShader(const ShaderMetaType::CompiledShaderInitializerType& Initializer) : FGlobalShader(Initializer)
	{
		...
		const FTextureRHIParamRef& TextureRHI)
	{
		...
		SetTextureParameter(RHICmdList, ShaderRHI, TextureVal, TextureSampler,
			TStaticSamplerState<SF_Trilinear, AM_Clamp, AM_Clamp, AM_Clamp>::GetRHI(), TextureRHI);
	}
...
public:
	...
	FSimplePixelShaderVS(const ShaderMetaType::CompiledShaderInitializerType& Initializer) : FSimplePixelShader(Initializer)
	{
	}
};
...
public:
	...
	FSimplePixelShaderPS(const ShaderMetaType::CompiledShaderInitializerType& Initializer) : FSimplePixelShader(Initializer)
	{
	}
};
...
public:
	...
	FSimpleComputeShader(const ShaderMetaType::CompiledShaderInitializerType& Initializer) : FGlobalShader(Initializer)
	{
	...
	void SetParameters(FRHICommandList& RHICmdList, FUnorderedAccessViewRHIParamRef InOutUAV, FSimpleUniformStruct UniformStruct)
	{
		...
		SetUniformBufferParameterImmediate(
			RHICmdList, GetComputeShader(), GetUniformBufferParameter<FSimpleUniformStructParameters>(), ShaderStructData);
	}
...
IMPLEMENT_SHADER_TYPE(
	, FSimpleComputeShader, TEXT("/Plugin/BRPlugins/Private/SimpleComputeShader.usf"), TEXT("MainCS"), SF_Compute);
...
static void DrawTestShaderRenderTarget_RenderThread(FRHICommandListImmediate& RHICmdList,
	FTextureRenderTargetResource* OutputRenderTargetResource, ERHIFeatureLevel::Type FeatureLevel,
	//    const FName TextureRenderTargetName,
	const FLinearColor MyColor, const FTextureRHIParamRef TextureRHI, const FSimpleUniformStruct UniformStruct)
{
...
#endif
	FRHIRenderPassInfo RPInfo(OutputRenderTargetResource->GetRenderTargetTexture(), ERenderTargetActions::DontLoad_Store,
	...
	SetUniformBufferParameterImmediate(RHICmdList, PixelShader->GetPixelShader(),
		PixelShader->GetUniformBufferParameter<FSimpleUniformStructParameters>(), Parameters);
	...
	RHICmdList.DrawIndexedPrimitive(GRectangleIndexBuffer.IndexBufferRHI,
		/*BaseVertexIndex=*/0,
		/*MinIndex=*/0,
		/*NumVertices=*/4,
		/*StartIndex=*/0,
...
static void UseComputeShader_RenderThread(FRHICommandListImmediate& RHICmdList,
	FTextureRenderTargetResource* OutputRenderTargetResource, FSimpleUniformStruct UniformStruct,
	ERHIFeatureLevel::Type FeatureLevel)
{
	...
	FTexture2DRHIRef Texture =
		RHICreateTexture2D(SizeX, SizeY, PF_A32B32G32R32F, 1, 1, TexCreate_ShaderResource | TexCreate_UAV, CreateInfo);
	...
	DrawTestShaderRenderTarget_RenderThread(
		RHICmdList, OutputRenderTargetResource, FeatureLevel, FLinearColor(), Texture, UniformStruct);
}
...
void USimplePixelShaderBlueprintLibrary::DrawTestShaderRenderTarget(const UObject* WorldContextObject,
	UTextureRenderTarget2D* OutputRenderTarget, FLinearColor MyColor, UTexture* MyTexture, FSimpleUniformStruct UniformStruct)
{
	...
	([TextureRenderTargetResource, FeatureLevel, MyColor, TextureRHI, UniformStruct](FRHICommandListImmediate& RHICmdList) {
		DrawTestShaderRenderTarget_RenderThread(
			RHICmdList, TextureRenderTargetResource, FeatureLevel, MyColor, TextureRHI, UniformStruct);
	});
}
...
void USimplePixelShaderBlueprintLibrary::TextureWriting(UTexture2D* TextureToBeWrite)
{
	...
	TextureToBeWrite->Source.Init(TextureToBeWrite->PlatformData->SizeX, TextureToBeWrite->PlatformData->SizeY, 1, 1,
		ETextureSourceFormat::TSF_BGRA8, (uint8*) Colors.GetData());
	TextureToBeWrite->UpdateResource();
}
...
void USimplePixelShaderBlueprintLibrary::UseComputeShader(
	const UObject* WorldContextObject, UTextureRenderTarget2D* OutputRenderTarget, FSimpleUniformStruct UniformStruct)
{
	...
	([TextureRenderTargetResource, FeatureLevel, UniformStruct](FRHICommandListImmediate& RHICmdList) {
		UseComputeShader_RenderThread(RHICmdList, TextureRenderTargetResource, UniformStruct, FeatureLevel);
	});
}
