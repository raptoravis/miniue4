...
{
	FString FullName = ObjectToLint->GetOutermost()->GetPathName();
	for (const FString& IgnoredPath : PathsIgnored)
	{
		if (FullName.StartsWith(IgnoredPath))
			return true;
	}
	...
	{
		bool bRegex = false;
		if (!Info.Prefix.IsEmpty() && Info.Prefix[0] == TCHAR('^'))
			bRegex = true;

		if (bRegex)
		{
			FRegexPattern RegexPattern(Info.Prefix);
			FRegexMatcher RegexMatcher(RegexPattern, ObjectToLint->GetName());
...
{
	if (!DesiredPrefix.IsEmpty() && DesiredPrefix[0] == TCHAR('^'))
